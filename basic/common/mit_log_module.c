...
MITLogFuncRetValue MITLogOpen(const char *appName, const char*logPath, int bufferMode)
{
    ...
    // alloc memory
    int i = MITLOG_INDEX_COMM_FILE;
    for (; i<= MITLOG_INDEX_ERROR_FILE; ++i) {
        logFilePaths[i] = (char *)calloc(MITLOG_MAX_FILE_NAME_PATH_LEN, sizeof(char));
        if (!logFilePaths[i]) {
            int j=MITLOG_INDEX_COMM_FILE;
            for (; j<i; ++j) {
                free(logFilePaths[j]);
                logFilePaths[j] = NULL;
            }
    ...
    for (; i<= MITLOG_INDEX_ERROR_FILE; ++i) {
        ...
        if (!originFilePointers[i]) {
            int j=MITLOG_INDEX_COMM_FILE;
            for (; j<=MITLOG_INDEX_ERROR_FILE; ++j) {
                free(logFilePaths[j]);
                logFilePaths[j] = NULL;
            }
            j=MITLOG_INDEX_COMM_FILE;
            for (; j<i; ++j) {
